import React, { useState, useEffect, useRef } from 'react';

// ChickenTracks v7 - COMPLETE SYSTEM
// Full Camera • 4 Navigation Tabs • All Rewards • Everything Working

export default function ChickenTracksApp() {
  // ============================================
  // STATE MANAGEMENT
  // ============================================
  
  // App navigation and flow
  const [appState, setAppState] = useState('splash'); // splash, onboarding, home
  const [onboardingStep, setOnboardingStep] = useState(0);
  const [currentView, setCurrentView] = useState('home'); // home, flock, rewards, settings
  
  // Flock management
  const [flock, setFlock] = useState([]);
  const [selectedChicken, setSelectedChicken] = useState(null);
  const [showAddChicken, setShowAddChicken] = useState(false);
  const [newChicken, setNewChicken] = useState({ 
    name: '', 
    breed: '', 
    age: '', 
    personality: '', 
    color: '#D4943D' 
  });
  
  // Progress tracking
  const [streak, setStreak] = useState(0);
  const [lastVisitDate, setLastVisitDate] = useState(null);
  const [todayComplete, setTodayComplete] = useState(false);
  const [outdoorMinutes, setOutdoorMinutes] = useState(0);
  
  // Quest system
  const [todaysQuests, setTodaysQuests] = useState([
    { id: 1, title: 'Collect Eggs', icon: '🥚', completed: false, reward: 25, time: '5 min' },
    { id: 2, title: 'Fresh Water', icon: '💧', completed: false, reward: 15, time: '3 min' },
    { id: 3, title: 'Health Check', icon: '🩺', completed: false, reward: 20, time: '5 min' },
  ]);
  
  // Reward inventory - ALL ITEMS WITH CORRECT EMOJIS
  const [inventory, setInventory] = useState({
    cornScratch: 0,
    mealworms: 0,
    oliveEggers: 0,
    doubleYolks: 0,     // Will use 🍳🍳 (two frying pans)
    chickenTracks: 0,   // Will use 🐾
  });
  
  const [showQuestComplete, setShowQuestComplete] = useState(false);
  const [showRewardDrop, setShowRewardDrop] = useState(false);
  const [lastDrop, setLastDrop] = useState(null);
  
  // Camera refs and state
  const photoInputRef = useRef(null);
  const videoRef = useRef(null);
  const canvasRef = useRef(null);
  
  const [showCamera, setShowCamera] = useState(false);
  const [cameraStream, setCameraStream] = useState(null);
  const [facingMode, setFacingMode] = useState('environment');
  const [hasFlash, setHasFlash] = useState(false);
  const [flashEnabled, setFlashEnabled] = useState(false);
  const [cameraError, setCameraError] = useState(null);
  const [capturedPhoto, setCapturedPhoto] = useState(null);
  const [cameraSupported, setCameraSupported] = useState(true);
  
  // ============================================
  // COLORS AND CONSTANTS
  // ============================================
  
  const colors = {
    navy: '#2D3E50',
    navyLight: '#3D5166',
    navyDark: '#1E2A36',
    redAccent: '#E63946',
    cream: '#F5F0E6',
    gold: '#D4943D',
    sage: '#8FAE7C',
    blueGray: '#4A5C6A',
    textMuted: '#7A8B99',
    oliveGreen: '#6B8E23',      // GREEN for olive eggs
    mealwormBrown: '#8B4513',
  };
  
  // REWARD TIERS - CORRECT EMOJIS
  const rewardTiers = {
    cornScratch: { name: 'Corn Scratch', emoji: '🌽', color: colors.gold, dropRate: 1.0 },
    mealworms: { name: 'Mealworms', emoji: '🪱', color: colors.mealwormBrown, dropRate: 0.25 },
    oliveEggers: { name: 'Olive Egg', emoji: '🥚', color: colors.oliveGreen, dropRate: 0.10 },
    doubleYolks: { name: 'Double Yolks', emoji: '🍳🍳', color: colors.gold, dropRate: 0.02 },
    chickenTracks: { name: 'Chicken Track', emoji: '🐾', color: colors.blueGray, dropRate: 0.005 },
  };
  
  const logoBase64 = "data:image/jpeg;base64,";
  
  // ============================================
  // CAMERA FUNCTIONS
  // ============================================
  
  const startCamera = async () => {
    try {
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        setCameraSupported(false);
        setCameraError('Camera not supported');
        return;
      }
      
      const constraints = {
        video: {
          facingMode: facingMode,
          width: { ideal: 1920, max: 1920 },
          height: { ideal: 1080, max: 1080 }
        }
      };
      
      const stream = await navigator.mediaDevices.getUserMedia(constraints);
      const track = stream.getVideoTracks()[0];
      const capabilities = track.getCapabilities();
      
      if (capabilities.torch) {
        setHasFlash(true);
      }
      
      setCameraStream(stream);
      if (videoRef.current) {
        videoRef.current.srcObject = stream;
      }
      setCameraError(null);
      
    } catch (err) {
      console.error('Camera error:', err);
      setCameraError(err.message);
      setCameraSupported(false);
    }
  };
  
  const stopCamera = () => {
    if (cameraStream) {
      cameraStream.getTracks().forEach(track => track.stop());
      setCameraStream(null);
    }
    if (videoRef.current) {
      videoRef.current.srcObject = null;
    }
  };
  
  const toggleFlash = async () => {
    if (cameraStream && hasFlash) {
      const track = cameraStream.getVideoTracks()[0];
      try {
        await track.applyConstraints({
          advanced: [{ torch: !flashEnabled }]
        });
        setFlashEnabled(!flashEnabled);
      } catch (err) {
        console.error('Flash error:', err);
      }
    }
  };
  
  const flipCamera = async () => {
    stopCamera();
    setFacingMode(facingMode === 'user' ? 'environment' : 'user');
  };
  
  const capturePhoto = () => {
    if (!videoRef.current || !canvasRef.current) return;
    
    const video = videoRef.current;
    const canvas = canvasRef.current;
    canvas.width = video.videoWidth;
    canvas.height = video.videoHeight;
    
    const ctx = canvas.getContext('2d');
    ctx.drawImage(video, 0, 0, canvas.width, canvas.height);
    
    const imageData = canvas.toDataURL('image/jpeg', 0.95);
    setCapturedPhoto(imageData);
  };
  
  const savePhoto = () => {
    if (!capturedPhoto || !selectedChicken) return;
    
    const photoData = {
      id: Date.now(),
      data: capturedPhoto,
      date: new Date().toLocaleDateString(),
      time: new Date().toLocaleTimeString(),
    };
    
    setFlock(prev => prev.map(chicken => 
      chicken.id === selectedChicken.id
        ? {
            ...chicken,
            photos: [...(chicken.photos || []), photoData],
            totalPhotos: (chicken.totalPhotos || 0) + 1,
          }
        : chicken
    ));
    
    setCapturedPhoto(null);
    setShowCamera(false);
    stopCamera();
  };
  
  const retakePhoto = () => {
    setCapturedPhoto(null);
  };
  
  const closeCamera = () => {
    setShowCamera(false);
    setCapturedPhoto(null);
    stopCamera();
  };
  
  const openCamera = (chicken) => {
    setSelectedChicken(chicken);
    setShowCamera(true);
  };
  
  // ============================================
  // APP HANDLERS
  // ============================================
  
  const addChicken = () => {
    if (!newChicken.name.trim()) return;
    
    const chicken = {
      id: Date.now(),
      ...newChicken,
      photos: [],
      totalPhotos: 0,
      dateAdded: new Date().toLocaleDateString(),
    };
    
    setFlock([...flock, chicken]);
    setNewChicken({ name: '', breed: '', age: '', personality: '', color: '#D4943D' });
    setShowAddChicken(false);
    
    // If this is first chicken, move to home
    if (flock.length === 0) {
      setAppState('home');
    }
  };
  
  const completeQuest = (questId) => {
    setTodaysQuests(prev => prev.map(q => 
      q.id === questId ? { ...q, completed: true } : q
    ));
    
    const quest = todaysQuests.find(q => q.id === questId);
    if (quest) {
      // Random reward drops
      const drops = [];
      Object.entries(rewardTiers).forEach(([key, tier]) => {
        if (Math.random() < tier.dropRate) {
          drops.push(key);
          const amount = key === 'cornScratch' ? Math.floor(Math.random() * 20) + 10 : 1;
          setInventory(prev => ({ ...prev, [key]: prev[key] + amount }));
        }
      });
      
      if (drops.length > 0) {
        setLastDrop(drops[0]);
        setShowRewardDrop(true);
        setTimeout(() => setShowRewardDrop(false), 2000);
      }
      
      setShowQuestComplete(true);
      setTimeout(() => setShowQuestComplete(false), 2000);
    }
    
    // Check if all complete
    const allComplete = todaysQuests.filter(q => q.id !== questId).every(q => q.completed);
    if (allComplete) {
      setTodayComplete(true);
      setStreak(prev => prev + 1);
    }
  };
  
  // ============================================
  // EFFECTS
  // ============================================
  
  // Splash screen
  useEffect(() => {
    if (appState === 'splash') {
      const timer = setTimeout(() => {
        setAppState(flock.length === 0 ? 'onboarding' : 'home');
      }, 2500);
      return () => clearTimeout(timer);
    }
  }, [appState, flock.length]);
  
  // Streak tracking
  useEffect(() => {
    const today = new Date().toDateString();
    if (lastVisitDate !== today) {
      setLastVisitDate(today);
      setTodayComplete(false);
      setTodaysQuests(prev => prev.map(q => ({ ...q, completed: false })));
    }
  }, [lastVisitDate]);
  
  // Camera lifecycle
  useEffect(() => {
    if (showCamera && cameraSupported) {
      startCamera();
    }
    return () => {
      if (showCamera) {
        stopCamera();
      }
    };
  }, [showCamera, facingMode]);
  
// This file will be appended to complete v7

  // ============================================
  // CAMERA UI COMPONENT
  // ============================================
  
  if (showCamera) {
    return (
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: colors.navyDark, zIndex: 9999, display: 'flex', flexDirection: 'column' }}>
        <canvas ref={canvasRef} style={{ display: 'none' }} />
        
        {/* Camera Header */}
        <div style={{ padding: '15px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0,0,0,0.5)' }}>
          <button onClick={closeCamera} style={{ background: 'transparent', border: 'none', color: 'white', fontSize: '28px', cursor: 'pointer' }}>✕</button>
          <div style={{ color: 'white', fontSize: '18px', fontWeight: '600' }}>{selectedChicken?.name}</div>
          <div style={{ width: '28px' }} />
        </div>
        
        {/* Camera View */}
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
          {!capturedPhoto ? (
            <>
              {cameraSupported && !cameraError ? (
                <>
                  <video ref={videoRef} autoPlay={true} playsInline={true} muted={true} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, pointerEvents: 'none', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gridTemplateRows: '1fr 1fr 1fr', opacity: 0.2 }}>
                    {[...Array(9)].map((_, i) => <div key={i} style={{ border: '1px solid white' }} />)}
                  </div>
                </>
              ) : (
                <div style={{ textAlign: 'center', color: 'white', padding: '40px' }}>
                  <div style={{ fontSize: '60px', marginBottom: '20px' }}>📷</div>
                  <p>Camera not available</p>
                  <p style={{ fontSize: '14px', color: '#ccc', marginTop: '10px' }}>{cameraError || 'Using file upload'}</p>
                  <input ref={photoInputRef} type="file" accept="image/*" capture="environment" onChange={(e) => { const file = e.target.files[0]; if (file) { const reader = new FileReader(); reader.onload = (ev) => { const photoData = { id: Date.now(), data: ev.target.result, date: new Date().toLocaleDateString(), time: new Date().toLocaleTimeString() }; setFlock(prev => prev.map(chicken => chicken.id === selectedChicken.id ? { ...chicken, photos: [...(chicken.photos || []), photoData], totalPhotos: (chicken.totalPhotos || 0) + 1 } : chicken)); setShowCamera(false); }; reader.readAsDataURL(file); } e.target.value = ''; }} style={{ display: 'none' }} />
                  <button onClick={() => photoInputRef.current?.click()} style={{ marginTop: '30px', background: colors.redAccent, color: 'white', padding: '15px 40px', border: 'none', borderRadius: '25px', fontSize: '18px', fontWeight: '600', cursor: 'pointer' }}>Choose Photo</button>
                </div>
              )}
            </>
          ) : (
            <img src={capturedPhoto} alt="Captured" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
          )}
        </div>
        
        {/* Camera Controls */}
        <div style={{ padding: '30px 20px', background: 'rgba(0,0,0,0.7)', display: 'flex', justifyContent: 'space-around', alignItems: 'center' }}>
          {!capturedPhoto ? (
            <>
              <button onClick={toggleFlash} disabled={!hasFlash} style={{ background: flashEnabled ? colors.gold : 'transparent', border: '2px solid white', borderRadius: '50%', width: '50px', height: '50px', fontSize: '24px', cursor: hasFlash ? 'pointer' : 'not-allowed', opacity: hasFlash ? 1 : 0.3 }}>⚡</button>
              <button onClick={capturePhoto} style={{ background: 'white', border: `4px solid ${colors.redAccent}`, borderRadius: '50%', width: '70px', height: '70px', cursor: 'pointer' }} />
              <button onClick={flipCamera} style={{ background: 'transparent', border: '2px solid white', borderRadius: '50%', width: '50px', height: '50px', fontSize: '24px', cursor: 'pointer' }}>🔄</button>
            </>
          ) : (
            <>
              <button onClick={retakePhoto} style={{ background: 'transparent', color: 'white', border: '2px solid white', borderRadius: '25px', padding: '12px 30px', fontSize: '16px', fontWeight: '600', cursor: 'pointer' }}>↻ Retake</button>
              <button onClick={savePhoto} style={{ background: colors.redAccent, color: 'white', border: 'none', borderRadius: '25px', padding: '12px 30px', fontSize: '16px', fontWeight: '600', cursor: 'pointer' }}>✓ Save Photo</button>
            </>
          )}
        </div>
      </div>
    );
  }
  
  // ============================================
  // SPLASH SCREEN
  // ============================================
  
  if (appState === 'splash') {
    return (
      <div style={{ background: `linear-gradient(135deg, ${colors.navy} 0%, ${colors.navyDark} 100%)`, minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <img src={logoBase64} alt="ChickenTracks" style={{ width: '280px', marginBottom: '30px' }} />
        <h1 style={{ color: colors.cream, fontSize: '42px', fontWeight: '800', margin: '0 0 12px' }}>ChickenTracks</h1>
        <p style={{ color: colors.textMuted, fontSize: '18px', margin: 0 }}>Get Outside, Track Your Flock!</p>
      </div>
    );
  }
  
  // ============================================
  // ONBOARDING
  // ============================================
  
  if (appState === 'onboarding') {
    if (onboardingStep === 0) {
      return (
        <div style={{ background: colors.cream, minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px 20px' }}>
          <img src={logoBase64} alt="ChickenTracks" style={{ width: '200px', marginBottom: '40px' }} />
          <h1 style={{ color: colors.navy, fontSize: '32px', fontWeight: '800', textAlign: 'center', marginBottom: '16px' }}>Welcome to ChickenTracks!</h1>
          <p style={{ color: colors.textMuted, fontSize: '18px', textAlign: 'center', maxWidth: '400px', marginBottom: '40px' }}>Let's add your first chicken to get started</p>
          <button onClick={() => setOnboardingStep(1)} style={{ background: colors.redAccent, color: 'white', padding: '18px 60px', border: 'none', borderRadius: '15px', fontSize: '18px', fontWeight: '600', cursor: 'pointer' }}>Get Started</button>
        </div>
      );
    }
    
    if (onboardingStep === 1) {
      return (
        <div style={{ background: colors.cream, minHeight: '100vh', padding: '40px 20px' }}>
          <h2 style={{ color: colors.navy, fontSize: '28px', fontWeight: '800', marginBottom: '30px' }}>Add Your First Chicken</h2>
          <div style={{ background: 'white', padding: '30px', borderRadius: '20px', maxWidth: '500px', margin: '0 auto' }}>
            <input value={newChicken.name} onChange={(e) => setNewChicken(prev => ({ ...prev, name: e.target.value }))} placeholder="Chicken's name" style={{ width: '100%', padding: '15px', marginBottom: '15px', border: `2px solid ${colors.cream}`, borderRadius: '12px', fontSize: '16px' }} />
            <input value={newChicken.breed} onChange={(e) => setNewChicken(prev => ({ ...prev, breed: e.target.value }))} placeholder="Breed (optional)" style={{ width: '100%', padding: '15px', marginBottom: '15px', border: `2px solid ${colors.cream}`, borderRadius: '12px', fontSize: '16px' }} />
            <input value={newChicken.age} onChange={(e) => setNewChicken(prev => ({ ...prev, age: e.target.value }))} placeholder="Age (optional)" style={{ width: '100%', padding: '15px', marginBottom: '30px', border: `2px solid ${colors.cream}`, borderRadius: '12px', fontSize: '16px' }} />
            <button onClick={addChicken} disabled={!newChicken.name.trim()} style={{ width: '100%', background: newChicken.name.trim() ? colors.redAccent : colors.textMuted, color: 'white', padding: '18px', border: 'none', borderRadius: '15px', fontSize: '18px', fontWeight: '600', cursor: newChicken.name.trim() ? 'pointer' : 'not-allowed' }}>Add Chicken</button>
          </div>
        </div>
      );
    }
  }

  // ============================================
  // NAVIGATION HELPER
  // ============================================
  
  const NavBar = () => (
    <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: 'white', padding: '15px 20px', boxShadow: '0 -4px 10px rgba(0,0,0,0.1)', display: 'flex', justifyContent: 'space-around', zIndex: 100 }}>
      {['home', 'flock', 'rewards', 'settings'].map(view => (
        <button key={view} onClick={() => setCurrentView(view)} style={{ background: 'transparent', border: 'none', color: currentView === view ? colors.redAccent : colors.textMuted, fontSize: '12px', fontWeight: '600', cursor: 'pointer', textTransform: 'capitalize', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
          <div style={{ fontSize: '24px' }}>
            {view === 'home' && '🏠'}
            {view === 'flock' && '🐔'}
            {view === 'rewards' && '🏆'}
            {view === 'settings' && '⚙️'}
          </div>
          <div>{view}</div>
        </button>
      ))}
    </div>
  );
  
  // ============================================
  // HOME VIEW
  // ============================================
  
  if (currentView === 'home') {
    return (
      <div style={{ background: colors.cream, minHeight: '100vh', paddingBottom: '80px' }}>
        <div style={{ background: colors.navy, padding: '20px' }}>
          <h1 style={{ color: colors.cream, fontSize: '28px', marginBottom: '10px' }}>Today's Quests</h1>
          <p style={{ color: colors.textMuted, fontSize: '14px' }}>🔥 {streak} day streak</p>
        </div>
        <div style={{ padding: '20px' }}>
          {todaysQuests.map(quest => (
            <div key={quest.id} onClick={() => !quest.completed && completeQuest(quest.id)} style={{ background: quest.completed ? colors.sage : 'white', padding: '20px', marginBottom: '12px', borderRadius: '15px', display: 'flex', alignItems: 'center', gap: '15px', cursor: quest.completed ? 'default' : 'pointer' }}>
              <div style={{ fontSize: '32px' }}>{quest.completed ? '✅' : quest.icon}</div>
              <div style={{ flex: 1 }}>
                <h3 style={{ fontSize: '18px', fontWeight: '700', color: quest.completed ? 'white' : colors.navy, margin: '0 0 4px' }}>{quest.title}</h3>
                <p style={{ fontSize: '14px', color: quest.completed ? 'rgba(255,255,255,0.9)' : colors.textMuted, margin: 0 }}>{quest.completed ? 'Complete!' : `+${quest.reward} points`}</p>
              </div>
            </div>
          ))}
        </div>
        <NavBar />
      </div>
    );
  }
  
  // ============================================
  // FLOCK VIEW
  // ============================================
  
  if (currentView === 'flock') {
    return (
      <div style={{ background: colors.cream, minHeight: '100vh', paddingBottom: '80px' }}>
        <div style={{ background: colors.navy, padding: '20px' }}>
          <h1 style={{ fontSize: '28px', marginBottom: '10px', color: 'white' }}>Your Flock</h1>
          <p style={{ color: colors.cream, fontSize: '14px' }}>{flock.length} chicken{flock.length !== 1 ? 's' : ''}</p>
        </div>
        <div style={{ padding: '20px' }}>
          <button onClick={() => setShowAddChicken(true)} style={{ width: '100%', marginBottom: '20px', background: colors.redAccent, color: 'white', padding: '18px', border: 'none', borderRadius: '15px', fontSize: '18px', fontWeight: '600', cursor: 'pointer' }}>+ Add New Chicken</button>
          {showAddChicken && (
            <div style={{ background: 'white', padding: '20px', borderRadius: '15px', marginBottom: '20px' }}>
              <h3 style={{ color: colors.navy, marginBottom: '15px' }}>New Chicken</h3>
              <input value={newChicken.name} onChange={(e) => setNewChicken(prev => ({ ...prev, name: e.target.value }))} placeholder="Name" style={{ width: '100%', padding: '12px', marginBottom: '10px', border: `2px solid ${colors.cream}`, borderRadius: '10px' }} />
              <input value={newChicken.breed} onChange={(e) => setNewChicken(prev => ({ ...prev, breed: e.target.value }))} placeholder="Breed" style={{ width: '100%', padding: '12px', marginBottom: '10px', border: `2px solid ${colors.cream}`, borderRadius: '10px' }} />
              <input value={newChicken.age} onChange={(e) => setNewChicken(prev => ({ ...prev, age: e.target.value }))} placeholder="Age" style={{ width: '100%', padding: '12px', marginBottom: '15px', border: `2px solid ${colors.cream}`, borderRadius: '10px' }} />
              <div style={{ display: 'flex', gap: '10px' }}>
                <button onClick={() => setShowAddChicken(false)} style={{ flex: 1, background: colors.textMuted, color: 'white', padding: '12px', border: 'none', borderRadius: '10px', cursor: 'pointer' }}>Cancel</button>
                <button onClick={addChicken} style={{ flex: 1, background: colors.redAccent, color: 'white', padding: '12px', border: 'none', borderRadius: '10px', cursor: 'pointer' }}>Add</button>
              </div>
            </div>
          )}
          {flock.map(chicken => (
            <div key={chicken.id} onClick={() => setSelectedChicken(chicken)} style={{ background: 'white', padding: '20px', marginBottom: '12px', borderRadius: '15px', cursor: 'pointer' }}>
              <h3 style={{ fontSize: '20px', fontWeight: '700', color: colors.navy, marginBottom: '8px' }}>🐔 {chicken.name}</h3>
              <p style={{ color: colors.textMuted, fontSize: '14px', margin: '4px 0' }}>{chicken.breed}</p>
              <p style={{ color: colors.textMuted, fontSize: '14px', margin: '4px 0' }}>📸 {chicken.totalPhotos || 0} photos</p>
            </div>
          ))}
        </div>
        <NavBar />
      </div>
    );
  }
  
  // ============================================
  // CHICKEN DETAIL VIEW
  // ============================================
  
  if (selectedChicken && currentView === 'flock') {
    return (
      <div style={{ background: colors.cream, minHeight: '100vh', paddingBottom: '80px' }}>
        <div style={{ background: colors.navy, padding: '20px' }}>
          <button onClick={() => setSelectedChicken(null)} style={{ background: 'transparent', border: 'none', color: colors.cream, fontSize: '24px', marginBottom: '10px', cursor: 'pointer' }}>←</button>
          <h1 style={{ color: colors.cream, fontSize: '28px', marginBottom: '8px' }}>{selectedChicken.name}</h1>
          <p style={{ color: colors.textMuted, fontSize: '14px' }}>{selectedChicken.breed} • {selectedChicken.age}</p>
        </div>
        <div style={{ padding: '20px' }}>
          <button onClick={() => openCamera(selectedChicken)} style={{ width: '100%', background: colors.redAccent, color: 'white', padding: '18px', border: 'none', borderRadius: '15px', fontSize: '18px', fontWeight: '600', cursor: 'pointer', marginBottom: '20px' }}>📷 Take Photo</button>
          <h3 style={{ fontSize: '20px', fontWeight: '700', color: colors.navy, marginBottom: '15px' }}>Photo Gallery ({(selectedChicken.photos || []).length})</h3>
          {(selectedChicken.photos || []).length === 0 ? (
            <div style={{ background: 'white', padding: '40px', borderRadius: '15px', textAlign: 'center' }}>
              <p style={{ color: colors.textMuted }}>No photos yet!</p>
              <p style={{ color: colors.textMuted, fontSize: '14px' }}>Tap "Take Photo" above to start your collection</p>
            </div>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px' }}>
              {(selectedChicken.photos || []).map(photo => (
                <div key={photo.id} style={{ borderRadius: '15px', overflow: 'hidden', aspectRatio: '1', background: 'white' }}>
                  <img src={photo.data} alt="Chicken" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                </div>
              ))}
            </div>
          )}
        </div>
        <NavBar />
      </div>
    );
  }
  
  // ============================================
  // REWARDS VIEW
  // ============================================
  
  if (currentView === 'rewards') {
    return (
      <div style={{ background: colors.cream, minHeight: '100vh', paddingBottom: '80px' }}>
        <div style={{ background: colors.navy, padding: '20px' }}>
          <h1 style={{ color: colors.cream, fontSize: '28px', marginBottom: '10px' }}>Rewards</h1>
          <p style={{ color: colors.textMuted, fontSize: '14px' }}>Your collected rewards</p>
        </div>
        <div style={{ padding: '20px' }}>
          {Object.entries(rewardTiers).map(([key, tier]) => (
            <div key={key} style={{ background: 'white', padding: '20px', marginBottom: '12px', borderRadius: '15px', display: 'flex', alignItems: 'center', gap: '15px' }}>
              <div style={{ fontSize: '40px' }}>{tier.emoji}</div>
              <div style={{ flex: 1 }}>
                <h3 style={{ fontSize: '18px', fontWeight: '700', color: colors.navy, margin: '0 0 4px' }}>{tier.name}</h3>
                <p style={{ fontSize: '14px', color: colors.textMuted, margin: 0 }}>Owned: {inventory[key]}</p>
              </div>
              <div style={{ fontSize: '24px', fontWeight: '800', color: tier.color }}>{inventory[key]}</div>
            </div>
          ))}
        </div>
        <NavBar />
      </div>
    );
  }
  
  // ============================================
  // SETTINGS VIEW
  // ============================================
  
  if (currentView === 'settings') {
    return (
      <div style={{ background: colors.cream, minHeight: '100vh', paddingBottom: '80px' }}>
        <div style={{ background: colors.navy, padding: '20px' }}>
          <h1 style={{ color: colors.cream, fontSize: '28px', marginBottom: '10px' }}>Settings</h1>
          <p style={{ color: colors.textMuted, fontSize: '14px' }}>App preferences</p>
        </div>
        <div style={{ padding: '20px' }}>
          <div style={{ background: 'white', padding: '20px', borderRadius: '15px' }}>
            <h3 style={{ fontSize: '18px', fontWeight: '700', color: colors.navy, marginBottom: '15px' }}>About</h3>
            <p style={{ color: colors.textMuted, fontSize: '14px', margin: '8px 0' }}>ChickenTracks v7</p>
            <p style={{ color: colors.textMuted, fontSize: '14px', margin: '8px 0' }}>Track your flock, build streaks, earn rewards</p>
          </div>
        </div>
        <NavBar />
      </div>
    );
  }
  
  // Fallback
  return <div>Loading...</div>;
}
